...
# Import the engine and SessionLocal from dependencies to avoid circular imports
...
def init_db():
    ...
    try:
        ...
        # Note: Tables are created by migrations. We don't call create_all() to avoid conflicts.
        logger.info("Tables will be created by migrations...")
        
        # Skip migrations for now since database is already set up
        logger.info("Skipping DB migrations (database already initialized)...")
        
        # Tables are already created by migrations, no need for create_all
...
def store_identity_embedding(db: Session | None, identity_id: str, embedding: np.ndarray = None):
    """
    Stores a new identity embedding in the database.

    This function is forgiving about the first argument for backward compatibility with
    older tests which call `store_identity_embedding(identity_id, embedding)`.

    Args:
        db: SQLAlchemy Session or None. If None, a session will be created using
            `SessionLocal()`.
        identity_id: Identifier for the identity.
        embedding: Numpy array embedding. If None, raises ValueError.
    """
    # Import model locally to avoid circular imports
    from .models import IdentityEmbedding

    # Detect when the function was called in the legacy form
    if embedding is None and not isinstance(db, Session):
        # Legacy call pattern: store_identity_embedding(identity_id, embedding)
    ...
    if embedding is None:
        raise ValueError("Embedding must be provided to store_identity_embedding")
...
def get_all_evidence(db: Session, skip: int = 0, limit: int = 100):
    """
    Retrieves all evidence records from the database (without decrypting content).
...
def delete_evidence(db: Session, evidence_id: int):
    """
    Deletes evidence from the database.
    ...
    evidence = db.query(Evidence).filter(Evidence.id == evidence_id).first()
    if not evidence:
        logger.warning(f"Evidence not found for deletion: {evidence_id}")
        return False
    
    db.delete(evidence)
    db.commit()
    logger.info(f"Deleted evidence {evidence_id}: {evidence.file_name}")
    return True
...
def load_watchlist_means(db: Session) -> dict[str, np.ndarray]:
    ...
    """
    # Prefer precomputed v2 files if present (produced by scripts/refresh_watchlist_embeddings.py)
    emb_path = Path('logs') / 'recognition' / 'watchlist_embeddings_v2.npy'
    labels_path = Path('logs') / 'recognition' / 'watchlist_labels_v2.json'
    if emb_path.exists() and labels_path.exists():
        try:
            labels = json.loads(labels_path.read_text())
            embs = np.load(emb_path)
            watchlist_means = {lab: embs[i] for i, lab in enumerate(labels)}
            logger.info(f"Loaded {len(watchlist_means)} watchlist identities from {emb_path}.")
            return watchlist_means
        except Exception as e:
            logger.warning(f"Failed to load v2 watchlist embeddings: {e}. Falling back to DB loader.")
